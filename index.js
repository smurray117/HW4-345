...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		//let newrepo = {"name": "NewRepo"};
		//await createRepo(userId, newrepo);
		//await createIssue(userId, "HW4-345", "CreateIssue", "You need to create an issue");
...
async function createIssue(owner,repo, issueName, issueBody)
{
	...
	options.data = JSON.stringify({"title": issueName, "body": issueBody});
...
async function enableWikiSupport(owner,repo)
{
	...
	options.data = JSON.stringify({"name":repo,"has_wiki":"true"});
